    ...
    for(key in columns){
        if (items[key]){                                                   //readings loop
            ...
            for(var i=1; i < b.length; i++){                               //chapters loop
                ...
                var sp = b[i].split(',');
                var dd; //double-dash
                for (v in sp){                                       //verses loop
                    var verse = sp[v].match(/\d+(-\d+)?/)[0];             
                    if (v != 0){
                        sqlselect += or;
                    }
                    if (dd === true){
            ...
            x += 1;
            eval("var dynamicVar" + x + "= {id:readingID};");   //"chubby" dynamic variable-objects for css classes after "tx.executeSql" due to asynchronous javascript
            if (book<=39){
